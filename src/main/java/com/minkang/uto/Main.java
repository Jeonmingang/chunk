...
public class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // Core services (protect against invalid config)
        try {
            this.tpsMonitor = new TpsMonitor(this);
        } catch (Throwable t) {
            getLogger().severe("[UTO] config.yml 파싱 오류로 TPS 설정 기본값(1/5/15s) 사용: " + t.getMessage());
            this.tpsMonitor = new TpsMonitor(this, 1, 5, 15);
        }
