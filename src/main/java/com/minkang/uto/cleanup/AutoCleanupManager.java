...
public class AutoCleanupManager {
    ...
    private void broadcastOnce(String path, String fallback, Map<String,String> vars) {
        ...
        Bukkit.getServer().broadcastMessage(ChatColor.translateAlternateColorCodes('&', plugin.prefix() + msg));
    }
    ...
    private void broadcastCountdown(int seconds, String typeKey) {
        ...
        vars.put("type", typeKey);
        broadcastOnce("messages.cleanup_countdown_map." + seconds, line, vars);
    }
    ...
    private void setupGroundCleanup() {
        ...
        groundTask = new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (t <= 0) {
                    broadcastStarted("바닥청소", worlds);

                    int total = 0;
                    for (World w : worlds) {
                        total += EntityBatchCleaner.cleanGroundItems(plugin, w);
                    }
                    final int totalRemoved = total;
                    broadcastDone("바닥청소", totalRemoved, worlds);
    ...
    private void setupPixelmonCleanup() {
        ...
        pixelTask = new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (t <= 0) {
                    broadcastStarted("비전설 픽셀몬 청소", worlds);
                    ...
                    if (plugin.getConfig().getBoolean("cleanup.broadcastOnce", true)) {
                        broadcastOnce("messages.pixelmon_cleanup_done", "&b비전설 픽셀몬 제거 완료: %count%개 (모든 월드)",
                                new HashMap<String,String>() {{ put("count", String.valueOf(totalRemoved)); }});
                    } else {
                        String base = plugin.getConfig().getString("messages.pixelmon_cleanup_done", "&b비전설 픽셀몬 제거 완료: %count%개 (%world%)");
                        for (World w : worlds) {
                            String msg = base.replace("%count%", String.valueOf(totalRemoved)).replace("%world%", w.getName());
    ...
    private void broadcastStarted(String typeKey, java.util.List<org.bukkit.World> worlds) {
        boolean once = plugin.getConfig().getBoolean("cleanup.broadcastOnce", true);
        if (once) {
            broadcastOnce("messages.cleanup_started", "&6모든 월드에서 %type% 를 시작합니다.",
                    new java.util.HashMap<String,String>() {{ put("type",typeKey); }});
        } else {
    ...
    private void broadcastDone(String typeKey, int totalRemoved, java.util.List<org.bukkit.World> worlds) {
        boolean once = plugin.getConfig().getBoolean("cleanup.broadcastOnce", true);
        if (once) {
            broadcastOnce("messages.cleanup_done", "&a%type% 완료: 제거된 엔티티 %count%개",
                    new java.util.HashMap<String,String>() {{ put("type",typeKey); put("count", String.valueOf(totalRemoved)); }});
        } else {
