...
public class AutoCleanupManager {
    ...
    private void startGroundCleaner() {
        ...
        boolean announceZero = plugin.getConfig().getBoolean("cleanup.ground.announceEvenIfZero", true);
    ...
    private void startPixelmonCleaner() {
        ...
        Set<String> worldSet = new HashSet<>(worlds);
        boolean announceZero = plugin.getConfig().getBoolean("cleanup.pixelmon.announceEvenIfZero", true);

        pixelmonTask = new BukkitRunnable() {
            @Override public void run() {
                for (World w : Bukkit.getWorlds()) {
                    if (!worldSet.isEmpty() && !worldSet.contains(w.getName())) continue;
                    int removed = PixelmonCleaner.cleanNonLegendary(plugin, w);
                    if (removed > 0 || announceZero) {
                        String msg = plugin.getConfig().getString("messages.pixelmon_cleanup_done");
                        if (msg != null) {
                            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
                                    plugin.prefix() + msg.replace("%count%", String.valueOf(removed))
