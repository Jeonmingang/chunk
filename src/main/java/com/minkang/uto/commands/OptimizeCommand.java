...
public class OptimizeCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if ("자동".equals(sub)) {
            if (args.length >= 2 && "실행".equals(args[1])) {
                ...
                sender.sendMessage(plugin.color(plugin.prefix()+"&a자동 최적화 실행 (조건 충족 시 카운트다운 후 동작)"));
                return true;
            }
            if (args.length >= 2 && "취소".equals(args[1])) {
                mgr.setAutoEnabledRuntime(false);
                sender.sendMessage(plugin.color(plugin.prefix()+"&e자동 최적화 취소 (다음 스케줄/카운트다운 중지)"));
                return true;
            }
            sender.sendMessage(plugin.color(plugin.prefix()+"&e/최적화 자동 실행 &7| &e/최적화 자동 취소"));
            return true;
        }
        ...
        if ("바닥청소".equals(sub)) {
            if (args.length >= 2) {
                ...
                if (w != null) {
                    mgr.triggerGroundOnce(java.util.Arrays.asList(w), plugin.getConfig().getInt("auto.countdownSeconds", 60));
                    sender.sendMessage(plugin.color(plugin.prefix()+"&a바닥청소 1회 실행 예약(월드: "+w.getName()+")"));
                    return true;
                }
            }
            sender.sendMessage(plugin.color(plugin.prefix()+"&e/최적화 바닥청소 on|off &7또는 &e/최적화 바닥청소 <월드>"));
            return true;
        }
        ...
        if ("픽셀몬".equals(sub)) {
            if (args.length >= 2) {
                ...
                World w = Bukkit.getWorld(args[1]);
                if (w != null) {
                    mgr.triggerPixelOnce(java.util.Arrays.asList(w), plugin.getConfig().getInt("auto.countdownSeconds", 60));
                    sender.sendMessage(plugin.color(plugin.prefix()+"&a픽셀몬 청소 1회 실행 예약(월드: "+w.getName()+")"));
                    return true;
                }
            }
            sender.sendMessage(plugin.color(plugin.prefix()+"&e/최적화 픽셀몬 on|off &7또는 &e/최적화 픽셀몬 <월드>"));
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            out.add("리로드"); out.add("상태"); out.add("자동"); out.add("바닥청소"); out.add("픽셀몬"); out.add("청키");
        } else if (args.length == 2 && "자동".equals(args[0])) {
            out.add("실행"); out.add("취소");
        } else if (args.length == 2 && "바닥청소".equals(args[0])) {
            out.add("on"); out.add("off");
            for (World w : Bukkit.getWorlds()) out.add(w.getName());
        } else if (args.length == 2 && "픽셀몬".equals(args[0])) {
            out.add("on"); out.add("off");
            for (World w : Bukkit.getWorlds()) out.add(w.getName());
