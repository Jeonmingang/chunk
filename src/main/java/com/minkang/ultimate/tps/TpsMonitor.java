...
public class TpsMonitor {
    ...
    private void push(Deque<Double> dq, double val, int max) {
        dq.addLast(val);
        while (dq.size() > max) dq.removeFirst();
    }

    private double avg(Deque<Double> dq) {
        if (dq.isEmpty()) return 20.0;
        double s=0; for (double v: dq) s+=v; return s/dq.size();
    }

    private double readTps() {
        // Paper: Bukkit.getServer().getTPS()
        try {
            Method m = Bukkit.getServer().getClass().getMethod("getTPS");
            double[] t = (double[]) m.invoke(Bukkit.getServer());
            // 1m, 5m, 15m 값이지만, 여기서는 1초 샘플로 1m 값을 사용
            return Math.min(20.0, t[0]);
        } catch (Throwable ignored) {}
        // Fallback: assume 20 (no precise spigot-only API). This still allows condition guard to function with defaults.
        return 20.0;
    }
