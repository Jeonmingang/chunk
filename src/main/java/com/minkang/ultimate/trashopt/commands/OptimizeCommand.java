...
public class OptimizeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            sender.sendMessage(ChatColor.GRAY + "/" + label + " 시작 [월드] [반경]  §7(Chunky+WorldBorder+Starlight 동시에)");
            sender.sendMessage(ChatColor.GRAY + "/" + label + " chunky 시작 [월드] [반경] | 일시중지 | 상태");
        ...
        // Shorthand: /최적화 시작 [월드] [반경]
        if ("시작".equalsIgnoreCase(args[0])) {
            String w = (args.length >= 2) ? args[1] : plugin.getConfig().getString("world-name", "world");
            int r = 3000;
            if (args.length >= 3) { try { r = Integer.parseInt(args[2]); } catch (NumberFormatException ignored) {} }
            if (!ConditionGuard.checkAllowed(sender, "최적화(all)")) return true;
            runTemplate(sender, "optimize.chunky.start", w, r);
            runTemplate(sender, "optimize.worldborder.start", w, r);
            runTemplate(sender, "optimize.starlight.relight", w, r);
        ...
        if ("worldborder".equals(tool)) {
            if ("시작".equals(action)) {
                ...
                // Use template; for API-based border control prefer automation or add a dedicated command later
                runTemplate(sender, "optimize.worldborder.start", worldName, radius);
                return true;
            } else if ("일시중지".equals(action)) {
    ...
    private void runTemplate(CommandSender sender, String path, String worldName, int radius) {
        ...
        for (String c : cmds) {
            String finalCmd = c.replace("{world}", worldName)
                               .replace("{radius}", String.valueOf(radius))
                               .replace("{diameter}", String.valueOf(radius * 2));
