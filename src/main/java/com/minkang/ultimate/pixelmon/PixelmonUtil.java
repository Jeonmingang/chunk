...
public class PixelmonUtil {
    public static boolean isPixelmon(Main plugin, Entity e) {
        ...
        String lowerName = com.minkang.ultimate.util.NmsUtil.tryGetHandleClassName(e).toLowerCase(Locale.ROOT);
        for (String s: contains) { if (lowerName.contains(s.toLowerCase(Locale.ROOT))) return true; }
        ...
        if (e instanceof LivingEntity) {
            ...
            if (name != null && Pattern.compile(pat).matcher(ChatColor.stripColor(name)).find()) return true;
        }
        return false;
    }
        ...
        if (e instanceof LivingEntity) {
            ...
            if (name != null) {
                ...
                for (String lg : plugin.getConfig().getStringList("cleanup.pixelmon.legendarySpeciesWhitelist")) {
                    if (s.toLowerCase(Locale.ROOT).contains(lg.toLowerCase(Locale.ROOT))) return true;
                }
                for (String tag: e.getScoreboardTags()) { if (tag.toLowerCase(Locale.ROOT).contains("legendary")) return true; }
            }
        }
        return false;
    }
