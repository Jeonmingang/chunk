...
public class CleanupCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) { sender.sendMessage(plugin.prefix() + "/cleanup <world> [seconds]"); return true; }
        World w = Bukkit.getWorld(args[0]);
        if (w == null) { sender.sendMessage(plugin.prefix() + ChatColor.RED + "월드를 찾을 수 없습니다: " + args[0]); return true; }
        int seconds = 10;
        if (args.length >= 2) { try { seconds = Math.max(0, Integer.parseInt(args[1])); } catch (Exception ignored) {} }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) { List<String> list = new ArrayList<>(); for (World w : Bukkit.getWorlds()) list.add(w.getName()); return list; }
        if (args.length == 2) return java.util.Arrays.asList("10","30","60","120","300");
