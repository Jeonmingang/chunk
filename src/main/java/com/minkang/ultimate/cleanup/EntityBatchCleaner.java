...
public class EntityBatchCleaner {
    public static int cleanWorldOnce(Main plugin, World world) {
        ...
        for (Entity e: world.getEntities()) {
            if (filter.test(e)) { try { e.remove(); removed[0]++; } catch (Throwable ignored) {} }
        }
        return removed[0];
    }
        ...
        new BukkitRunnable(){
            ...
            @Override public void run() {
                ...
                while (idx < all.length && count < batchPerTick) {
                    Entity e = all[idx++];
                    if (predicate.test(e)) { try { e.remove(); removed++; } catch (Throwable ignored) {} }
                    count++;
                }
                if (idx >= all.length) {
                    cancel();
                    if (onFinish != null) Bukkit.getScheduler().runTask(plugin, onFinish);
                }
            }
