...
public class ConditionGuard {
    ...
    public boolean isEnabled() { return plugin.getConfig().getBoolean("conditions.enabled", true); }
    public boolean checkAllowed(CommandSender feedbackTarget) {
        ...
        if (m != null && m.getShortAvg() < minTps) reason = "TPS " + String.format("%.2f", m.getShortAvg()) + " < " + minTps;
        if (reason == null) {
            int maxPlayers = plugin.getConfig().getInt("conditions.max_players", 50);
            int online = Bukkit.getOnlinePlayers().size();
            if (online > maxPlayers) reason = "Players " + online + " > " + maxPlayers;
        }
        if (reason == null && plugin.getConfig().getBoolean("conditions.time_window.enabled", false)) {
            ...
            int h = now.getHour(); boolean in;
            if (start <= end) in = (h >= start && h < end); else in = (h >= start || h < end);
        ...
        if (reason == null) {
            ...
            if (days != null && !days.isEmpty()) {
                ...
                boolean ok = false; for (String d: days) if (d.equalsIgnoreCase(today)) { ok = true; break; }
        ...
        if (reason == null) {
            ...
            if (last > 0 && (nowMs - last) < cd*1000L) reason = "Cooldown " + ((cd*1000L - (nowMs - last))/1000) + "s left";
        }
        if (reason != null) {
            ...
            if (feedbackTarget != null) {
                ...
                feedbackTarget.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.prefix() + msg.replace("%reason%", reason)));
            }
            return false;
        } else {
            if (feedbackTarget != null) {
