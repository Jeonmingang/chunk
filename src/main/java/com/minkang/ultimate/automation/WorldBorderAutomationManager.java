...
public class WorldBorderAutomationManager {
    ...
    public void initFromConfig() {
        ...
        task = new BukkitRunnable() {
            @Override public void run() {
                ...
                for (String wname : sec.getKeys(false)) {
                    ...
                    int stepSec = sec.getInt(wname + ".step_seconds", 60);
                    int cur = radiusState.getOrDefault(wname, 0);
                    if (cur == 0) cur = Math.min(1000, inc); // init
                    if (cur >= max) continue;
                    ...
                    if (!dryRun) {
                        // Fire WorldBorder commands if plugin exists
                        if (Bukkit.getPluginManager().getPlugin("WorldBorder") != null) {
                            // Basic example: center remains default (0,0)
        ...
        task.runTaskTimer(plugin, 20L, 20L * 60); // tick every 60s, internal step_seconds is respected per world in commands
    }
